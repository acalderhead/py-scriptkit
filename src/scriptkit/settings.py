...
Purpose
───────
    Dataclass-driven configuration with CLI + env wiring. Subclass
    ``ScriptSettings`` in a script and add only that script's fields; each
    settable field automatically becomes a CLI flag and reads a matching
    environment variable (see ENV_PREFIX).
...
───────
    ...
    Supported field types are wired automatically:
    - scalars (str, int, float, Path, and anything else constructible
               from a string, e.g. Decimal / UUID)
    - ``bool`` (as a ``--flag`` / ``--no-flag`` pair)
    - ``datetime`` / ``date`` (ISO 8601 via ``fromisoformat``)
    - ``Enum`` and ``Literal[...]`` (constrained choices)
    - ``list[X]`` (repeatable on the CLI, comma-separated in the env)
    - ``Optional[...]`` wrapping any of the above

    Derived (init = False) fields and types that can't map to a single CLI
    argument (dict, tuple, non-Optional unions) are skipped and left to bespoke
    handling.

Public API
──────────
    ENV_PREFIX                 : Env-var prefix for overrides (default "APP_")
...
Usage
─────
    from scriptkit.settings import ScriptSettings, parse_settings
"""
